...
| [M4-04](milestones/M4/tasks/M4-04-sentry-release-sourcemap-upload.md) | 配置 Release（发布版本）与私有 Source Map 上传 | frontend-team + sre-team | P0 | `done`（[ADR-0006](decisions/0006-sentry-release-sourcemap-strategy.md)；Release `0.0.0-2ce306b` 已上传验证） | 构建与上传脚本 | Sentry 能把压缩堆栈还原到源码文件和行号 |
| M4-05 | 增加 Breadcrumbs（操作轨迹）与环境上下文 | frontend-team | P1 | `paused` | 错误上下文字段 | Issue 包含操作轨迹、环境、版本和场景 |
...
| M4-07 | 实现简化 symbolication（源码反解） | backend-team | P1 | `done`（`2075dd1` GlitchTip 端到端 sourcemap 反解 + `b46e08e` glitchtip-tool） | 演示用反解 API | 压缩 JS 行列号能通过私有 Source Map 映射到源码位置 |
| M4-08 | 对比 Sentry 与自研反解流程 | docs-team | P1 | `done`（随 M4-07 一并完成，见 [ADR-0002](decisions/0002-error-tracking-strategy.md) + [ADR-0010](decisions/0010-glitchtip-bearer-direct-integration.md)） | 架构与安全说明 | 明确生产方案、教学方案和 Source Map 不公开原则 |
